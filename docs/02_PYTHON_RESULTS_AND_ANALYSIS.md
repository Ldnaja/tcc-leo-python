...
| baseline_both      | greedy_backlog                  |                192.094 |            0.725 |             0.838 |           0.252 |                     15.560 |
| baseline_queue_cap | greedy_backlog                  |                190.190 |            0.122 |             0.989 |           0.251 |                    217.334 |
| baseline_timeout   | greedy_backlog                  |                192.644 |            0.720 |             1.000 |           0.251 |                     16.378 |
...
No entanto, goodput não foi a única dimensão considerada. Por isso, a análise detalhada concentrou-se no cenário `baseline_both`, que foi adotado como cenário principal por combinar simultaneamente fila máxima e timeout.
...
Essa combinação de resultados justifica a divisão interpretativa adotada ao longo do desenvolvimento:
...
A integração com RL foi feita por etapas, produzindo quatro versões de PPO. A Tabela 4 resume a evolução operacional dessas versões.
...
A Tabela 4 e as Figuras 6–9 mostram a evolução das versões do PPO ao longo do processo de desenvolvimento. Entretanto, essa comparação exige uma ressalva metodológica importante: as versões `ppo_v1`, `ppo_v2`, `ppo_v3` e `ppo_v4` não compartilham exatamente a mesma definição de função de recompensa. Houve uma formulação inicial na `ppo_v1`, uma segunda formulação na `ppo_v2` e uma formulação final compartilhada pelas versões `ppo_v3` e `ppo_v4`.

Por esse motivo, a recompensa média por episódio não deve ser interpretada como métrica principal de comparação absoluta entre todas as versões. A comparação transversal mais adequada deve ser feita pelas métricas operacionais do ambiente, como goodput, bloqueio, aceitação, serviço e atraso. Nesse sentido, observa-se uma evolução consistente do desempenho: o goodput médio passou de 153.171 Mbps na `ppo_v1` para 159.496 Mbps na `ppo_v4`, acompanhado por leve redução do bloqueio final, aumento da aceitação média e crescimento do serviço médio.

Esse comportamento indica que houve ganho real de política ao longo das versões, e não apenas uma alteração de escala ou formulação da recompensa. O avanço nas métricas operacionais sugere que o agente passou a tomar decisões mais adequadas ao objetivo de controle do ambiente, ainda que parte da variação na recompensa entre versões esteja associada às mudanças na própria definição da reward.

A comparação direta pela recompensa média é mais apropriada no recorte entre `ppo_v3` e `ppo_v4`, pois ambas utilizam a mesma formulação de reward. Nesse caso, a recompensa média subiu de 74.362 para 77.459, enquanto o goodput médio aumentou de 156.229 Mbps para 159.496 Mbps. Isso reforça que a `ppo_v4` apresentou melhoria sob o mesmo critério de otimização da `ppo_v3`.

Os boxplots complementam essa leitura ao mostrar que a `ppo_v4` não apenas obteve maior média de goodput, mas também operou em um patamar superior entre as seeds, mantendo dispersão controlada. Já a figura de reward entre todas as versões deve ser interpretada como apoio diagnóstico do processo de treinamento, e não como um ranking absoluto entre versões com funções de recompensa diferentes.
...
Como a robustez da `ppo_v4` usa a mesma reward final das versões `ppo_v3` e `ppo_v4`, a coluna de recompensa pode ser lida aqui como indicador interno complementar. Ainda assim, a interpretação principal deve permanecer ancorada nas métricas operacionais do ambiente.
...
Esse comportamento é desejável, porque indica que o agente não aprendeu apenas um caso específico, mas reagiu consistentemente às mudanças de regime do sistema.
...
### 8.3 O PPO evoluiu de modo consistente nas métricas do ambiente

A transição entre `ppo_v1` e `ppo_v4` mostrou ganho contínuo em goodput. A leitura da reward deve ser feita com cautela entre formulações diferentes, mas o avanço de `v3` para `v4` também se refletiu em reward mais alta sob a mesma definição, o que reforça a melhora do agente final.
