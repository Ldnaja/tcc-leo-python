...
\midrule
proportional\_fair & 157.628 & 0.772 & 0.823 & 0.210 & 15.336 & 0.819 & 61.3 \\
...
greedy\_backlog & 192.094 & 0.725 & 0.838 & 0.252 & 15.560 & 0.985 & 62.4 \\
\bottomrule
\end{tabular}
\end{table}
```
...
\midrule
both\_ch32 & 106.565 & 0.840 & 0.814 & 0.140 & 15.576 \\
both\_ch64 & 205.125 & 0.713 & 0.832 & 0.270 & 14.951 \\
both\_int3 & 177.466 & 0.747 & 0.827 & 0.235 & 15.233 \\
both\_int6 & 138.453 & 0.798 & 0.820 & 0.183 & 15.409 \\
both\_q5\_t10 & 162.300 & 0.781 & 0.967 & 0.211 & 7.332 \\
both\_q5\_t40 & 152.222 & 0.772 & 0.592 & 0.205 & 30.113 \\
both\_q10\_t20 & 159.252 & 0.767 & 0.994 & 0.207 & 16.157 \\
both\_q15\_t20 & 159.308 & 0.767 & 1.000 & 0.207 & 16.187 \\
\bottomrule
\end{tabular}
\end{table}
```

## Tabela adicional — Configuração das versões PPO

```latex
\begin{table}[htbp]
\centering
\caption{Resumo de construção das versões PPO.}
\label{tab:ppo_build_versions}
\begin{tabular}{llll}
\toprule
Versão & Arquivo & Alteração principal & Observação \\
\midrule
ppo\_v1 & ppo\_both.yaml & primeira configuração estável & 30000 timesteps, reward v1 \\
ppo\_v2 & ppo\_both\_v2.yaml & aumento do treino & 300000 timesteps, reward v2 \\
ppo\_v3 & ppo\_both\_v3.yaml & refinamento do PPO & 350000 timesteps, lr 0.0002, ent\_coef 0.003, reward final \\
ppo\_v4 & ppo\_both\_v4.yaml & BC + ajuste final & reward final, 250000 timesteps, expert + bc \\
\bottomrule
\end{tabular}
\end{table}
```
...
\begin{table}[htbp]
\centering
\caption{Resumo das versões PPO avaliadas. A reward não é estritamente comparável entre todas as versões, pois houve mudanças de formulação entre v1, v2 e a reward final compartilhada por v3 e v4.}
...
\midrule
both\_ch32 & -100.649 & 107.447 & 0.841 & 0.812 & 0.140 & 14.409 \\
...
both\_q15\_t20 & 69.749 & 159.097 & 0.769 & 1.000 & 0.207 & 14.815 \\
\bottomrule
\end{tabular}
\end{table}
```

> Observação: como a robustez usa a mesma reward final da ppo\_v4, a coluna de recompensa pode ser lida como indicador complementar interno, mas a interpretação principal deve continuar baseada em goodput, bloqueio, aceitação, serviço e atraso.
