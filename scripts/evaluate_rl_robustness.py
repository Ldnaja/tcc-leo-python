...
def main() -> None:
    robustness_cfg_path = Path("configs/rl/robustness_v4.yaml")
    with open(robustness_cfg_path, "r", encoding="utf-8") as f:
        robustness_cfg = yaml.safe_load(f)

    rl_model_config = Path(robustness_cfg["rl_model_config"])
    with open(rl_model_config, "r", encoding="utf-8") as f:
        rl_cfg = yaml.safe_load(f)

    queue_norm = float(rl_cfg["rl"]["queue_norm"])
    goodput_norm = float(rl_cfg["rl"]["goodput_norm"])
    throughput_norm = float(rl_cfg["rl"]["throughput_norm"])
    model_path = rl_cfg["paths"]["model_path"]

    out_dir = Path(robustness_cfg["paths"]["out_dir"])
    out_dir.mkdir(parents=True, exist_ok=True)

    seeds = robustness_cfg["seeds"]
    scenario_paths = [Path(x) for x in robustness_cfg["scenarios"]]

    model = PPO.load(model_path, device="cpu")

    rows = []

    total_runs = len(seeds) * len(scenario_paths)
    run_idx = 0
    ...
    for scenario_path in scenario_paths:
        ...
        for seed in seeds:
            run_idx += 1
            print(f"[{run_idx}/{total_runs}] cenário={scenario_name} seed={seed}")
            ...
            run_dir = out_dir / "runs" / scenario_name / f"seed_{seed}"
            run_dir.mkdir(parents=True, exist_ok=True)

            pd.DataFrame(history).to_csv(run_dir / "history.csv", index=False)
            env.scenario.save_plots(run_dir)

            with open(run_dir / "summary.json", "w", encoding="utf-8") as f:
                json.dump(row, f, indent=2, ensure_ascii=False)

    runs_df = pd.DataFrame(rows)
    runs_df.to_csv(out_dir / "rl_robustness_runs.csv", index=False)
    ...
    summary_df.to_csv(out_dir / "rl_robustness_summary.csv", index=False)

    print("\n--- RESUMO ROBUSTEZ RL V4 ---")
